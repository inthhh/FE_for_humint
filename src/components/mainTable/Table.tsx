  ...
  return (
    <div>
      <Provider store={store}>
        ...
        <div className="filter-wrap">
          ...
          <div className="filter-required">
            <SelectDate />
            <SelectSiteCode />
            <button className="btn-type btn-filter" onClick={() => handleFilter()}>
              테이블 보기
            </button>
            {/* 검색 입력창 및 '검색' 버튼 */}
            <div className="search-wrap">
              ...
              />
              <button onClick={() => searchAPI()} className="btn-type btn-search">
                검색
              </button>
            </div>
          </div>
        ...
        <table {...getTableProps()}>
          ...
          {dataList && dataList.length > 0 ? (
                        ...
                        <React.Fragment key={index}>
                          {index === row.cells.length - 4 ? ( // Check 열에 라디오 버튼 생성
                            ...
                            <>
                              <td>
                                <button
                                  onClick={() => handleButtonClick(ri, dataList[ri].id)}
                                  className="btn-type btn-save"
                                >
                                  저장
                                </button>
                                {isSaved && (
            ...
            <tbody>
              <tr>
                <td colSpan={13}>
                  <div className="no-data">테이블 값이 없습니다</div>
                </td>
              </tr>
            </tbody>
          )}
      ...
      {isIframeOpen && iframeSrcs ? (
        <ImgIframe src={iframeSrcs} imgDesc={iframeDesc} imgArea={iframeArea} onClose={handleIframeClose} />
      ) : null}
      <ScrollToTopBtn />
    </div>
  );
